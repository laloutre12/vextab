...
Vex.Flow.TabDiv.prototype.init = function(sel, options) {
  ...
  if (Vex.Flow.Player) {
    this.player = new Vex.Flow.Player(this.artist, options);
  }
...
Vex.Flow.TabDiv.start = function(opts) {
  var instances = [];
  var elements = $(Vex.Flow.TabDiv.SEL);
  for (var i=0; i<elements.length; i++) {
    elem = elements[i];
    instances[i] = new Vex.Flow.TabDiv(elem, opts);
  }
  return instances;
};